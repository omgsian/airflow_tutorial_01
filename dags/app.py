...
def get_amazon_data_books(num_books, ti):
    ...
    base_url = f"https://www.amazon.com/s?k=user+experience+books"
...
def insert_book_data_into_postgres(ti):
    book_data = ti.xcom_pull(key="book_data", task_ids="retrieve_amazon_books")
    if not book_data:
...
dag = DAG(
    "retrieve_amazon_books",
    default_args=default_args,
    description="DAG to retrieve list of books from Aamzon.com and store it in Postgres",
    schedule_interval=timedelta(days=1),
)

fetch_book_data_task = PythonOperator(
    task_id="retrieve_amazon_books",
